...
class SAMImageProcessor:
    ...
    def process_image(self, image_path, output_path, monitoring_path=None, mask_override_dict=None, point_override_dict=None):
        ...
        try:
            ...
            # Use point override if provided, else use center point
            if point_override_dict and image_name in point_override_dict:
                center_point = np.array([point_override_dict[image_name]])
                print(f"Using override point {point_override_dict[image_name]} for {image_name}")
            else:
            ...
            if monitoring_path:
                self._create_monitoring_visualization(image_rgb, masks, scores, monitoring_path, center_point, selected_mask_index)
...
def process_images(input_folder, output_folder, max_images=None, mask_override_dict=None, point_override_dict=None, csv_path=None):
    os.makedirs(output_folder, exist_ok=True)
    monitoring_folder = os.path.join(output_folder, "monitoring")
    os.makedirs(monitoring_folder, exist_ok=True)

    csv_status = read_csv_status(csv_path) if csv_path else {}
    ...
    for i, image_path in enumerate(unprocessed_files, 1):
        ...
        success, processing_time, _ = processor.process_image(
            image_path, output_path, monitoring_path, mask_override_dict, point_override_dict
        )
        if success:
            successful += 1
...
def main():
    ...
    POINT_OVERRIDE_DICT = {
        # Example: "IMG_1234": [150, 200],
        # "DSC_5678": [300, 400]
    }

    process_images(
        args.input, args.output, args.max,
        MASK_OVERRIDE_DICT, POINT_OVERRIDE_DICT, args.csv
    )
...
if __name__ == "__main__":
    ...
    MASK_OVERRIDE_DICT = {}
    POINT_OVERRIDE_DICT = {}
    process_images(INPUT_FOLDER, OUTPUT_FOLDER, MAX_IMAGES, MASK_OVERRIDE_DICT, POINT_OVERRIDE_DICT, CSV_PATH)
