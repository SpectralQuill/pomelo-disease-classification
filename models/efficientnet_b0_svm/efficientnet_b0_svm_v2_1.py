...
class EffB0SVMTrainer:
    ...
    def build_finetune_model(self, base_model, num_classes, dropout_rate=0.4):
        """Build fine-tuning model with custom head (v1.0 approach)"""
        x = base_model.output
    ...
    def fine_tune_effb0(self):
        """Two-phase training approach (v1.0 strategy)"""
        # Build base model
        if self.base_model is None:
            base = self.build_efficientnet_base()
        else:
        ...
        # Compile
        lr = float(self.fine_tune_cfg.get("learning_rate", 1e-4))
        model.compile(
        ...
        # Prepare in-memory datasets (v1.0 approach for better performance)
        ...
        ckpt_path = self.weights_dir / "effb0_phase1_best.keras"
        ...
        # PHASE 2: Fine-tune with some base layers unfrozen
        print("PHASE 2: Fine-tuning base layers")
        base.trainable = True
        
        # Unfreeze top layers (v1.0 strategy)
        fine_tune_at = int(len(base.layers) * 0.75)
        for layer in base.layers[:fine_tune_at]:
            layer.trainable = False

        # Recompile with lower learning rate
        lr_ft = float(self.fine_tune_cfg.get("fine_tune_learning_rate", lr / 10))
        model.compile(
        ...
        ckpt_path_ft = self.weights_dir / "effb0_phase2_best.keras"
        mc_ft = callbacks.ModelCheckpoint(
            str(ckpt_path_ft),
            monitor=es_cfg.get("monitor", "val_accuracy"),
            save_best_only=True,
            save_weights_only=False
        ...
        epochs_ft = int(self.fine_tune_cfg.get("fine_tune_epochs", 10))
        ...
        # Create feature extractor (use the base model with pooling='avg')
        self.feature_extractor = models.Model(
            inputs=base.input,
            outputs=base.output  # This is already GlobalAveragePooling2D output
        )
        ...
        try:
            ...
            print(f"Warning: Could not save feature extractor: {e}")
