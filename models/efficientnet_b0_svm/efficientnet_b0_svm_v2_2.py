...
class EffB0SVMTrainer:
    ...
    def build_finetune_model(self, base_model, num_classes):
        """Improved fine-tuning model with balanced classifier head."""
        x = base_model.output
        if len(x.shape) == 4:
        ...
        x = layers.BatchNormalization()(x)
        x = layers.Dropout(self.model_cfg.get("efficientnet", {}).get("dropout_rate", 0.4))(x)
    ...
    def fine_tune_effb0(self):
        """Two-phase fine-tuning of EfficientNet-B0 Hybrid without addons"""

        print("Starting fine-tuning (EfficientNet-B0 Hybrid)...")

        # Load or build base model
        ...
        lr_phase2 = float(self.fine_tune_cfg.get("fine_tune_learning_rate", lr_phase1 / 10))
        optimizer_ft = optimizers.Adam(learning_rate=lr_phase2)
        ...
        ckpt_phase2 = self.weights_dir / "effb0_phase2_best.keras"
        ...
        history2 = model.fit(
            X_train_pp, y_train_enc,
            validation_data=(X_val_pp, y_val_enc),
            epochs=int(self.fine_tune_cfg.get("fine_tune_epochs", 10)),
            batch_size=int(self.fine_tune_cfg.get("batch_size", self.batch_size)),
            callbacks=[es, reduce_lr, mc2],
            class_weight=class_weights,
            verbose=1
        )
        ...
        print("Saved fine-tuned model to:", self.weights_dir / "effb0_finetuned_full_model.keras")

        # Create feature extractor (base output for SVM)
