...
class EffB0SVMTrainer:
            ...
            if n_needed > 0:
                ...
                while saved < n_needed:
                    ...
                    try:
                        ...
                        print(f"  Keras augmentation failed for {src}: {e}")
                        continue
    ...
    def fine_tune_effb0(self):
        """Three-stage fine-tuning of EfficientNet-B0 Hybrid (gradual unfreezing)."""
        ...
        X_train, y_train_enc = self.load_images_to_memory(self.train_files, self.train_labels)
        X_val, y_val_enc = self.load_images_to_memory(self.val_files, self.val_labels)
        ...
        history1 = model.fit(
            ...
            batch_size=int(self.fine_tune_cfg.get("batch_size", self.batch_size)),
        ...
        # -------------------------------
        print("\nPHASE 2: Gradual fine-tuning of base layers")

        unfreeze_top_n = int(self.model_cfg.get("unfreeze_top_n_layers", 30))
        lr_phase2_start = float(self.fine_tune_cfg.get("fine_tune_learning_rate", lr_phase1 / 10))
        epochs_phase2 = int(self.fine_tune_cfg.get("fine_tune_epochs", 10))
        batch_size = int(self.fine_tune_cfg.get("batch_size", self.batch_size))

        if unfreeze_top_n <= 0:
            print("Warning: unfreeze_top_n_layers <= 0, skipping fine-tuning.")
            return

        total_layers = len(base.layers)
        half_n = unfreeze_top_n // 2

        # ---- Phase 2a: Unfreeze top N/2 layers ----
        print(f"Unfreezing top {half_n} layers (out of {total_layers})...")
        base.trainable = True
        for layer in base.layers[:-half_n]:
            layer.trainable = False

        optimizer_ft1 = optimizers.Adam(learning_rate=lr_phase2_start)
        model.compile(optimizer=optimizer_ft1,
                    loss="sparse_categorical_crossentropy",
                    metrics=["accuracy"])

        ckpt_phase2a = self.weights_dir / "effb0_phase2a_best.keras"
        mc2a = callbacks.ModelCheckpoint(
            str(ckpt_phase2a),
            monitor=es_cfg.get("monitor", "val_accuracy"),
            save_best_only=True,
            verbose=1
        ...
        print("Fine-tuning with top half of layers unfrozen...")
        history2a = model.fit(
            X_train_pp, y_train_enc,
            validation_data=(X_val_pp, y_val_enc),
            epochs=max(epochs_phase2 // 2, 1),
            batch_size=batch_size,
            callbacks=[es, reduce_lr, mc2a],
            class_weight=class_weights,
            verbose=1
        )

        # ---- Phase 2b: Unfreeze top N layers ----
        print(f"\nUnfreezing top {unfreeze_top_n} layers (final fine-tuning)...")
        for layer in base.layers[:-unfreeze_top_n]:
            layer.trainable = False

        optimizer_ft2 = optimizers.Adam(learning_rate=lr_phase2_start / 2)
        model.compile(optimizer=optimizer_ft2,
                    loss="sparse_categorical_crossentropy",
                    metrics=["accuracy"])

        ckpt_phase2b = self.weights_dir / "effb0_phase2b_best.keras"
        mc2b = callbacks.ModelCheckpoint(
            str(ckpt_phase2b),
            monitor=es_cfg.get("monitor", "val_accuracy"),
            save_best_only=True,
            verbose=1
        ...
        print("Final fine-tuning with smaller learning rate...")
        history2b = model.fit(
            X_train_pp, y_train_enc,
            validation_data=(X_val_pp, y_val_enc),
            epochs=max(epochs_phase2 - (epochs_phase2 // 2), 1),
            batch_size=batch_size,
            callbacks=[es, reduce_lr, mc2b],
            class_weight=class_weights,
            verbose=1
        )
        ...
        self.history = {
            'accuracy': history1.history['accuracy']
                        + history2a.history['accuracy']
                        + history2b.history['accuracy'],
            'val_accuracy': history1.history['val_accuracy']
                            + history2a.history['val_accuracy']
                            + history2b.history['val_accuracy'],
            'loss': history1.history['loss']
                    + history2a.history['loss']
                    + history2b.history['loss'],
            'val_loss': history1.history['val_loss']
                        + history2a.history['val_loss']
                        + history2b.history['val_loss']
        }
        ...
        print("\nFine-tuning complete (gradual unfreezing).")
