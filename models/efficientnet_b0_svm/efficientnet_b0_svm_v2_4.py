...
from tensorflow.keras import layers, models, callbacks, mixed_precision, optimizers, regularizers
...
class EffB0SVMTrainer:
    ...
    def build_finetune_model(self, base_model, num_classes):
        """Enhanced classifier head with deeper mixed activations and stronger regularization."""
        x = base_model.output
        if len(x.shape) == 4:
    ...
    def fine_tune_effb0(self):
        """Progressive multi-phase fine-tuning of EfficientNet-B0 Hybrid (no addons, memory-safe)."""

        print("Starting fine-tuning (EfficientNet-B0 Hybrid v2)...")

        # === Build base model and classifier ===
        ...
        img_size = int(self.model_cfg.get("image_size", 224))
        ...
        def build_dataset(file_list, label_list, shuffle=False):
        ...
        train_ds = build_dataset(train_files, y_train_enc, shuffle=True)
        val_ds = build_dataset(val_files, y_val_enc)
        ...
        hist1 = model.fit(
            ...
            callbacks=[early_stop, reduce_lr, mc1],
            verbose=1
        )

        for k in history_all:
            history_all[k] += hist1.history[k]

        # === Phases 2–4: Progressive unfreezing ===
        unfreeze_schedule = self.model_cfg.get("unfreeze_schedule", [])
        for i, phase in enumerate(unfreeze_schedule, start=2):
            unfreeze_n = int(phase.get("unfreeze_top_n_layers", 30))
            lr = float(phase.get("lr", 1e-5))
            epochs = int(phase.get("epochs", 10))

            print(f"\nPHASE {i}: Unfreezing top {unfreeze_n} layers | LR={lr} | Epochs={epochs}")

            base.trainable = True
            for layer in base.layers[:-unfreeze_n]:
                layer.trainable = False

            # Cosine decay restarts (optional)
            if self.fine_tune_cfg.get("cosine_decay_restarts", True):
                steps_per_epoch = max(1, len(self.train_files) // batch_size)
                ...
                optimizer_ft = optimizers.Adam(learning_rate=lr_schedule)
            else:
                optimizer_ft = optimizers.Adam(learning_rate=lr)
            ...
            for k in history_all:
                history_all[k] += hist.history[k]

        # === Save results ===
        self.history = history_all
        model.save(self.weights_dir / "effb0_finetuned_full_model.keras")
        print("Saved final fine-tuned model:", self.weights_dir / "effb0_finetuned_full_model.keras")
