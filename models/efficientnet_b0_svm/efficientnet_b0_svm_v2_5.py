...
class EffB0SVMTrainer:
    ...
    def fine_tune_effb0(self):
        ...
        # Data loading mode
        # -------------------------------
        data_mode = self.fine_tune_cfg.get("data_loading_mode", "stream").lower()
        ...
        if data_mode == "preload":
            ...
            print("→ Using streaming dataset loader (tf.data)...")
            train_ds, val_ds = self.build_datasets_from_files()

        # -------------------------------
        # Callbacks Setup (common)
        # -------------------------------
        ...
        if data_mode == "preload":
            hist1 = model.fit(
                ...
                batch_size=int(self.fine_tune_cfg.get("batch_size", 16)),
                verbose=1
            )
        else:
            hist1 = model.fit(
        ...
        # -------------------------------
        # PHASES 2...N: Gradual unfreezing
        # -------------------------------
        unfreeze_top_n = int(self.fine_tune_cfg.get("unfreeze_top_n_layers", 60))
        n_subphases = int(self.fine_tune_cfg.get("unfreeze_phases", 3))
        epochs_per_phase = int(self.fine_tune_cfg.get("fine_tune_epochs", 12)) // n_subphases
        lr_phase2_start = float(self.fine_tune_cfg.get("fine_tune_learning_rate", lr_phase1 / 10))

        unfreeze_step = max(1, unfreeze_top_n // n_subphases)
        print(f"\nGradual unfreezing across {n_subphases} sub-phases, {unfreeze_step} layers each...")

        for i in range(n_subphases):
            phase_idx = i + 2
            unfreeze_limit = unfreeze_step * (i + 1)
            lr_current = lr_phase2_start / (2 ** i)

            print(f"\nPHASE {phase_idx}: Unfreezing top {unfreeze_limit} layers | LR={lr_current:.1e} | Epochs={epochs_per_phase}")

            base.trainable = True
            for layer in base.layers[:-unfreeze_limit]:
                layer.trainable = False

            optimizer_ft = optimizers.Adam(learning_rate=lr_current)
            model.compile(
            ...
            ckpt_phase = self.weights_dir / f"effb0_phase{phase_idx}_best.keras"
            mc_phase = callbacks.ModelCheckpoint(
                str(ckpt_phase),
                monitor=es_cfg.get("monitor", "val_accuracy"),
                save_best_only=True,
                verbose=1
            ...
            if data_mode == "preload":
                histX = model.fit(
                    ...
                    train_ds[1],
                    validation_data=val_ds,
                    epochs=max(epochs_per_phase, 1),
                    callbacks=[es, mc_phase, reduce_lr],
                    class_weight=class_weights,
                    batch_size=int(self.fine_tune_cfg.get("batch_size", 16)),
                    verbose=1
                )
            ...
            history_all.append(histX.history)
        ...
        print("\nFine-tuning complete (multi-phase gradual unfreezing).")
