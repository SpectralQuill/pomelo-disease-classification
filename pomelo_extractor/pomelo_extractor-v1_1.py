...
class CitrusDetector:
    ...
    def process_image(self, image_path):
        ...
        # If no citrus detected, return None
        if citrus_detections.empty:
            return None, list(detected_classes) if not detections.empty else []
        ...
        return cropped_img, list(detected_classes)
    ...
    def write_log_entry(self, file_handle, image_name, detected_classes, success):
        """Write log entry for a single image"""
    ...
    def process_all_images(self):
        ...
        with open(self.log_file_path, 'w') as log_file:
            ...
            for image_path in image_files:
                ...
                # Process image
                cropped_img, detected_classes = self.process_image(image_path)
                
                # Check if detection was skipped due to size
                original_detections = set(detected_classes) & self.citrus_classes
                if not original_detections and any(cls in detected_classes for cls in self.citrus_classes):
                    skipped_small_detections += 1
                    print(f"  ⚠ Citrus detected but too small - skipped")
                
                # Log results
                success = cropped_img is not None
                self.write_log_entry(log_file, image_path.name, detected_classes, success)
